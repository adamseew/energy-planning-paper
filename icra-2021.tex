...
\begin{figure}[t]
  \centering
  \input{\figpath/il_abs.tikz}
  \input{\figpath/il_abs_energy.tikz}
  \caption{An initial plan of an agricultural scenario replanned dynamically in terms of both the path (\ref{sth:i}~travels a more accurate survey than~\ref{sth:ii}) and computations (\ref{sth:a}~elaborates more images per second than~\ref{sth:b}). In bottom right, the collected energy data of the UAV flying the scenario, along the power spectrum (small rectangle) where the first frequency has been filtered.}
  \label{fig:il-abs}
\end{figure}
...
The algorithm inputs an user-specified initial plan that consist of different stages. At each stage the UAV flies a trajectory, and does some computations. Per each stage the plan contains some parameters that allow to change the path and computations along an energy budget. These parameters are bounded. There is one trajectory constraints set which bounds the path and multiple computations constraints sets, one per each computation parameter, which bound computations. In Figure~\ref{fig:il-abs}, there are two parameters. The radius of the trajectory circle is relative to the trajectory (\ref{sth:i}~and~\ref{sth:ii}). The quality of image detection is relative to the computations (\ref{sth:a}~and~\ref{sth:b}). 
...
\begin{figure}[h]
  ...
  \caption{Illustrative figure shows of the alteration of the trajectory parameter $c_{1,1}$, the radius of the circle. The figure shows the alteration of the path from Figure~\ref{fig:il-abs}.}
  \label{fig:tee1}
\end{figure}
...
\end{algorithmic}
line~\ref{code:teeset} corresponds to the trajectory $\varphi_1$'s contraints set $\mathbb{C}_1=[-3\cdot10^3,0]$. The algorithm selects $c_{1,1}\in\mathbb{C}_1$. Line~\ref{code:qos1set} corresponds to the computation constraints set $\mathbb{S}_{1,1}$ for the computation parameter $c_{1,2}$. Line~\ref{code:qos1set} corresponds to the computation constraints set $\mathbb{S}_{1,2}$ for the computation parameter $c_{1,2}$. Lines~\ref{code:teeset}--\ref{code:qos2set} must follow the ascending order of the parameters. Lines~\ref{code:firstcircle}--\ref{code:qos2set} all describe the stage $\Gamma_1$.
