...
The remaining sections of the letter are then organized as follows. Section~\ref{sec:prob} provides basic constructs. These include the concepts of %computations and motion energies, 
...
Figure \ref{fig:state-machine-loop} illustrates  the concepts in Definition~\ref{def:primitive}. A plan composed of $n$ stages $\Gamma_1,\Gamma_2,\dots,\Gamma_n$ (containing primitive paths $\varphi_1,\varphi_2,\dots,\varphi_n$) is reiterated with the shift $\mathbf{d}$. $t_1<\dots<t_4$ are time instant $\in\mathbb{R}_{\geq 0}$. $\Gamma_f$ is the accepting stage, indicating the plan is completed, $\Gamma_0$ the initial stage where the aerial robot, e.g., awaits the starting command.
...
For a set $\mathbb{X}$, the notation $|\mathbb{X}|$ indicates the cardinality of $\mathbb{X}$%, i.e, $|\mathbb{X}|:=\sum_{x\in\mathbb{X}}1$
.
...
%\end{proof}
%Appendix~\ref{app:proof-eqv} contains a proof of Lemma~\ref{lem:eqv}.
...
%\end{align}
%\end{subequations}
%where $(x_\mathbf{p},y_\mathbf{p})=:\mathbf{p}$ for any point $\mathbf{p}$, and $\lfloor\,\cdot\,\rfloor$ is the integer division (it has a different meaning with parameters set $c_i$, see Equation~\ref{eq:piece-wise-reg}). The circles are
...
%\end{align}
%\end{subequations}
%where $r_1,r_2\in\mathbb{R}_{>0}$ s.t. $r_1>r_2>\underline{r}$ are radiuses. $r_1=r_2+x_\mathbf{d}/2$, whereas $r_2$ is in Equation~(\ref{eq:r2}).
...
\begin{algorithm}[t]
  \begin{algorithmic}[1]
    ...
    \FORALL{$i\in\mathcal{T}$}
      ...
      \WHILE{$b_d(C\mathbf{q}(k))>0$}\vspace*{.3ex}
        \IF{$k\in\mathcal{K}$}
...
%\end{split}\end{equation*}
%where $\mathrm{diag}(C)$ is a diagonal matrix with the items of $C$ from Equation~(\ref{eq:state-details}).
...
\begin{figure*}
  ...
  \begin{subfigure}{.4\textwidth}
    \centering
    \footnotesize
    \input{\figpath/results/new_physics2/trajs2.tikz}
  ...
  \begin{subfigure}{.54\textwidth}
    \centering
    \footnotesize
    \input{\figpath/results/new_physics2/ener2.tikz}
...
%The algorithm that we presented in this paper is motivated by a periodic behavior of empirical data on energy consumption (see the subfigures of Figure~\ref{fig:il-abs}). 
...
%We implemented a realistic simulator to simulate the empirical data of a given plan and atmospheric conditions. We show the effects of different conditions in Figure~\ref{fig:trajs}. 
...
Figure~\ref{fig:ener} %~\hyperref[fig:ener:static-I]{6.I}--\hyperref[fig:ener:static-II]{6.II} 
...
The figure further details the energy model (see detail view for \hyperref[fig:ener:static-I]{I}--\hyperref[fig:ener:static-II]{II}) on an initial slice of the model ($C\hat{\mathbf{q}}(i)$), power ($y(i)$), and period ($T$).% We motivate this choice again with Figure~\ref{fig:il-abs}, where the power spectrum subfigure shows that 3 frequencies are adequate. 
...
%Subfigures \hyperref[fig:trajs-dyn-i]{5.i}--\hyperref[fig:trajs-dyn-ii]{5.ii} in Figure~\ref{fig:trajs} path-wise, and \hyperref[fig:ener-dyn-i]{6.i} and \hyperref[fig:ener-dyn-ii]{6.ii} of Figure~\ref{fig:ener} energy-wise. For the first path (Subfigure \hyperref[fig:trajs-I-static]{5.I}) the plan starts at the highest configuration of parameters. 
