...
\end{equation}
where $\underline{c}_k,\overline{c}_k$ are the bounds from $\mathcal{M}$ that disclose the are over which the TEE $\varphi_k:\mathbb{R}^2\times\mathbb{R}^{\sigma}\rightarrow\mathbb{R}$ is free to evolve using controls $\mathbf{c}_k:=c_{k,1},...,c_{k,\sigma}$.
...
where $y_k\in\mathbb{R}_{\geq 0}$ is the instantaneous energy consumption. We prove formally in the Theorem~\ref{thm:state-vs-energy} the instantaneous energy being obtained as a linear combination of the state. The state $\mathbf{q}$ mimics the original Fourier series coefficients, and
...
\begin{equation}\label{eq:state-control}\begin{split}  
  \mathbf{u}_k&=\begin{bmatrix}g(\text{QoS}_k)-g(\text{QoS}_{k-1}) & \mathbf{c}_k-\mathbf{c}_{k-1}\end{bmatrix}^T\\
  ...
  \end{array}\right]
\end{split}\end{equation}
where $\text{QoS}_k$ is defined in Subsection~\ref{sec:computations-model}, $\mathbf{u}_k\in\mathbb{R}^l$ is the control given $l:=1+\rho$, $\mathbf{u}_{-1}=\begin{bmatrix}0 & \cdots & 0\end{bmatrix}$ and $B\in\mathbb{R}^{j\times l}$. Moreover, the first item on the diagonal is one, while the others are gain factors $\omega_{k}\in\mathbb{R}$, quantifying the contribution of a given TEE parameter to the instantaneous energy. Equation~(\ref{eq:state-control}) accounts for the energy due to the computations. The energy due to the change of explicit trajectory equation parameters is merely a linear combination of the gain factor and the value of the TEE. Nevertheless, the change updates the trajectory which will hence affect the reading from the sensors and adjust the energy evolution accordingly.
