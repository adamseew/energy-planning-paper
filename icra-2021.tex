...
\begin{algorithmic}[1]
  \Procedure{Step}{$\mathcal{M}_{k-1}$, $\mathbf{q}_{k-1}$, $\mathbf{u}_{k-1}^a$, $\mathbf{u}_{k-2}^a$, $P_{k-1}$}\label{alg:step}
  ...
  \If{$C\hat{\mathbf{q}}_k^-\notin\mathbb{Y}_k$}\label{alg:output_constraints}
    ...
    \State\Return{\Call{Step}{$\mathcal{M}_{k-1}$, $\mathbf{q}_{k-1}$, $\mathbf{u}^a_{k-1}$, $\mathbf{u}_{k-2}^a$,$P_{k-1}$}}
  \Else
    ...
    \State\Return{$(\hat{\mathbf{q}}_k,\mathbf{u}_k^a)$}
  \EndIf
  %\State $r\gets a\bmod b$
  %\State $\mathcal{M}_1\gets a\bmod b$
  ...
  \EndProcedure
  \Procedure{EADMPA}{$\mathcal{M}_0$, $\mathbf{p}_0$, $\mathbf{q}_0$}
  \State $k\gets 0$
%  \State $\mathbf{y}\gets \{\emptyset\}$
  \State $\mathbf{u}_{k-1}^a\gets\{\emptyset\}$
  \State $\mathbf{u}_{k}^a\gets \{(\mathbf{c}_0,\mathbf{s}_0)\mid(\varphi_0(\mathbf{p}_{0},\mathbf{c}_{0}),\Psi(\mathbf{s_{0}}))\in\mathcal{M}_{0}\}$
  \State $\mathcal{M}_k\gets\mathcal{M}_0$
  \State $\mathbf{p}_k\gets\mathbf{p}_0$
  \State $\mathbf{q}_k\gets\mathbf{q}_0$
  \While{$k \leq t_f$}
    
    \State $(\mathbf{q}_k,\mathbf{u}_k^a)\gets$\Call{Step}{$\mathbf{q}_{k}$, $\mathbf{u}^a_{k}$, $\mathbf{u}_{k-1}^a$}
    \State $\mathbf{p}_{k}=\mathbf{p}_k\dot{\mathbf{p}}_d(\mathbf{p}_k)/v$
...
%The algorithm per each time step updates the state. The position at line
  
Line~\ref{alg:max_cont_sequence} selects the maximum possible control from the control sequence at the time step $k-1$. Line~\ref{alg:mpc} uses robust output feedback model predictive control (MPC)~\cite{rawlings2017model} to select the optimal control $\mathbf{u}^0$ for a given horizon $N$ from the cost function
\begin{equation}\begin{split}
...
Follows a check if the mission can finish without the eventuality of the battery discharge (output constraints satisfaction) at line~\ref{alg:output_constraints}. The control sequence is then eventually updated.

Before the algorithm returns the energy evolution and the control sequence, a state estimator--the discrete-time Kalman filter~\cite{simon2006optimal} (lines~\ref{alg:kalman_start}--\ref{alg:kalman_end})--predicts the state $\mathbf{q}$ if the modeled instantaneous energy consumption diverging from the sensor's value $y_k^m\in\mathbb{R}$ more than a given $\varepsilon\in\mathbb{R}_{\geq  0}$.
