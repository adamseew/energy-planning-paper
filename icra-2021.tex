...
Let us further adopt the following mathematical notation.  $[a]$ is the set $\{0,1,\dots,a\}$, $[a]^+$ the set $[a]/\{0\}$, $\langle a_1,a_2,...,a_n\rangle$ an ordered list of $n$ elements, and $\underline{c},\overline{c}$ the lower and upper bound of the parameter $c$ retrieved from a plan specification. The plan specification is a file which specifies all the stages and triggering points along the plan. We discuss the format in Subsection~\ref{sec:plan}.
...
\begin{defn}[Stage, plan and triggering point]\label{def:mission}
  ...
  The \emph{plan} is then a function which maps a point to a specific stage $\Gamma_i$. The point $\mathbf{p}_{\Gamma_{i}}$ that allows the transition between $\Gamma_i$ and $\Gamma_{i+1}$ is called \emph{triggering point}.
  \begin{figure}[h]
  \center
  \begin{tikzpicture}[shorten >=1pt,node distance=12ex,on grid,auto] 
    ...
    (q_dots0.south west) -- (q_dots1.south east) node [black,midway,yshift=-8ex]{$\Gamma:\mathbb{R}^2\rightarrow\Gamma_i$};
  \end{tikzpicture}
  ...
  The algorithm takes as input $\Gamma,\bigcup_i{\Gamma_i}$, the triggering points (using the plan specification) and some initial guesses that are necessary for the position and energy estimation. It outputs the new position, the instantaneous energy consumption, and the control input--an action performed evolving the state.
\end{defn}
...
\end{algorithmic}
where $x$ and $y$ are the two coordinates of the point. The UAV reaches the point within a given radius (a parameter of the algorithm) and enters the next stage $\Gamma_2$.
