...
Let us consider for practical reasons the discretized version $A_d$ of the matrix $A$. We denote the continuous and discrete time with the standard notation (such as $t,k$).
...
\end{thm}
\begin{proof}
  From Definition~\ref{def:valid} we know that there exists a $\mathcal{J}\subseteq[l]^+$ where the path is valid, and from Proposition~\ref{prop:rank} we know that the path is periodic. Hence if the set $\mathcal{J}$ is valid, then also the set $i[j-n+1,j]\subseteq[l]^+$ is valid for a given $i$. From Assumption~\ref{assm:valid} we note that $i\in[l/n]^+$ and thus under the assumptions $\mathcal{J}=[l]^+$. Due to the periodicity, Lemma~\ref{lem:eqv} tells us that we can use the proposed approach to model the energy.
\end{proof}

We can thus output a valid control sequence for the initial plan $\Gamma$. For practical means, we consider some realistic constraints to the energy of a flying UAV. We illustrate such constraints in the following subsection.
