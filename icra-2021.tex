...
\begin{equation}\label{eq:fourier}
  h(k)=a_0/T+(2/T)\sum_{j=1}^{r}{\left(a_j\cos{\omega jk}+b_j\sin{\omega jk}\right)},
\end{equation}
...
\begin{equation}\label{eq:state-details}\begin{split}
  ...
  A&=\left[\begin{array}{cccc}
    1&    &       &  \\
     ...
     &    & \ddots&  \\
     &    &       & A_r 
  \end{array}\right],\,A_j\begin{bmatrix}1 & \omega j \\ -\omega j & 1\end{bmatrix},\\
...
\end{lem}
\begin{proof}
We re-write the Fourier series expression in Equation~(\ref{eq:fourier}) in its complex form with the well-known Euler's formula $e^{it}=\cos{t}+i\sin{t}$. With $t=\omega jk$, we find the expression for $\cos{\omega jk}=(e^{i\omega jk}+e^{-i\omega jk})/2$ and $\sin{\omega jk}=(e^{i\omega jk}-e^{-i\omega jk})/(2i)$ by substitution of $\sin{\omega jk}$ and $\cos{\omega jk}$ respectively. This leads~\cite{kuo1967automatic}
\begin{equation}\begin{split}\label{eq:proof-complex}
  h(k)=a_0/T+&(1/T)\sum_{j=1}^{r}{e^{i\omega jk}(a_j-ib_j)}+\\&(1/T)\sum_{j=1}^{r}{e^{-i\omega jk}(a_j+ib_j)},
 \end{split}\end{equation}
where $i$ is the imaginary unit. 

Let us consider the continuous form of Equation~(\ref{eq:state-perf}), $\dot{\mathbf{q}}=A_c\mathbf{q}$, and an initial guess $\mathbf{q}_0$. $A_c$ is defined as the continuous version of matrix $A$ in Equation~(\ref{eq:state-details}); $A_c:=A-I$, where $I$ is a properly sized identity matrix. The solution at time $k$ can be expressed $\mathbf{q}=e^{A_ck}\mathbf{q}_0$. Both the solution and the system in Equation~(\ref{eq:state-perf}) are well established expressions derived using a standard textbooks~\cite{kuo1967automatic, ogata2002modern}. To solve the matrix exponential $e^{A_ck}$, we use the eigenvectors matrix decomposition method~\cite{moler2003nineteen}.

The method works on the similarity transformation $A_c=VDV^{-1}$. The power series definition of $e^{A_ck}$ implies $e^{A_ck}=Ve^{Dk}V^{-1}$~\cite{moler2003nineteen}. We consider the non-singular matrix $V$, whose columns are eigenvectors of $A_c$; $V:=\begin{bmatrix}v_0 & v_1^0 & v_1^1 & \dots & v_r^0 & v_r^1\end{bmatrix}$. We then consider the diagonal matrix of eigenvalues $D=\mathrm{diag}{(\lambda_0,\lambda_1^0,\lambda_1^1,\dots,\lambda_r^0,\lambda_r^1)}$, where $\lambda_j$ is the $j$-th eigenvalue of $A_c$. $\lambda_j^0,\lambda_j^1$ are the two eigenvalues associated with the block $A_j-I$. We can write $A_cv_j=\lambda_jv_j\,\,\,\forall j=\{1,\dots,m\}$, and thus $AV=VD$. 

We apply the approach in terms of the continuous form of Euqation~(\ref{eq:state-perf}), $\dot{\mathbf{q}}=A_c\mathbf{q}$. Initial guess is expressed $\mathbf{q}_0=\mathbf{\gamma}V$ as a combination of eigenvectors. The generic solution is $\mathbf{q}_k=\mathbf{\gamma}e^{Dk}V$.

Let us express the output at time $k$ of the continuous equivalent of the model in Equation~(\ref{eq:state-perf}) as a linear combination of the state $y=C\mathbf{q}$
\begin{equation}\label{eq:proof-output}
  y=\gamma_0e^{\lambda_0k}v_0+\sum_{j=1}^r{\gamma_je^{\lambda_j^0k}v_j^0}+\sum_{j=1}^r{\gamma_je^{\lambda_j^1k}v_j^1},
\end{equation}
...
The matrix $A_c$ is a block diagonal matrix, and we can express its determinant as the multiplication of the determinants of its blocks $\det{(A_c)}=\det{(0)}\times\det{(A_1-I)}\times\cdots\times\det{(A_r-I)}$. We proof each block separately.

First, we proof that the first term of both the Equation~(\ref{eq:proof-complex}) and~(\ref{eq:proof-output}) matches. We find the eigenvalue from $\det(0-\lambda I)=0$, which is $\lambda_0=0$. The corresponding eigenvector can be chosen arbitrarily $(0-\lambda_0)v_0=0\,\,\,\forall v_0$, thus we choose $v_0=1$. We find the value $\gamma_0$ of the vector $\gamma$ so that the terms are equal $a_0/T=\gamma_0$. 

Then, we proof that all the terms in the sum of both the Equations~(\ref{eq:proof-complex}) and~(\ref{eq:proof-output}) match. 

For the first block $A_1-I$, we find the eigenvalues from $\det(A_1-(1+\lambda)I)=0$. The polynomial $\lambda^2+\omega^2$, gives two complex roots--the two eigenvalues $\lambda_1^0=i\omega$ and $\lambda_1^1=-i\omega$. The eigenvector associated with the eigenvalue $\lambda_1^0$ is $v_1^0=\begin{bmatrix}-i&1\end{bmatrix}^T$. The eigenvector associated with the eigenvalue $\lambda_1^1$ is $v_1^1=\begin{bmatrix}i&1\end{bmatrix}^T$. Again, we find the subvector $\gamma_1$ of the vector $\gamma$ such that the equivalences $(1/T)e^{i\omega k}(a_1-ib_1)=\gamma_1e^{i\omega k}\begin{bmatrix}-i&1\end{bmatrix}^T$ and $(1/T)e^{-i\omega k}(a_1+ib_1)=\gamma_1e^{i\omega k}\begin{bmatrix}i&1\end{bmatrix}^T$ hold. They hold for $\gamma_1=(1/T)\begin{bmatrix}b_1&a_1\end{bmatrix}$.

The proof for the remaining $r-1$ blocks is equivalent.

The discretized version $A$ of the matrix $A_c$ can be expressed $A=A_c+I$ for small enough values of time step (forward Euler approximation\footnote{One can guarantee to have the same outputs with $A=e^{A_ck}$}). We thus conclude that the lemma holds.
