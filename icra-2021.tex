...
\begin{defn}[Valid control sequence and path]\label{def:valid}
  Given a close strictly positive discrete interval $[j-n+1,j]$, a \emph{control sequence} $\mathbf{u}$ is \emph{valid} on $j$ if for every stage $\Gamma_{i-1},\,i\in[j-n+1,j]$ there exists a control $\mathbf{u}_k$ that produce the next stage $\Gamma_i$. 
  
  The path $\mathcal{P}$ resulting from such control sequence is a \emph{valid path} on $j$
  \begin{equation*}\begin{split}\label{eq:mission-valid}
   \mathcal{P}:=\{\mathbf{p}_{k}\mid\,\exists \mathbf{u}_k,&\forall i\in[j-n+1,j]\,\\&\langle\varphi_i(\mathbf{p}_{k},c_{k}^\rho),c_{k}^\sigma\rangle\in\Gamma_i\}.
  \end{split}\end{equation*}
\end{defn}
