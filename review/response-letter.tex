...
We thank the editor for handling our manuscript on planning-scheduling for the aerial robotics domain and the summary of the main concerns from the two reviews. We have incorporated all of the points and made numerous modifications to improve the readability. Regarding the related work, we have updated the manuscript to include detailed commentary on how our approach differs. 

We thus believe that with these edits, we have significantly improved the clarity of the technical contribution of our work and would like to bring the revised version for your kind consideration.}
...
We have updated the introduction and the problem formulation in Section II to address the observation and further added an explanatory text in Section IV-B as well as in the conclusion. In the introduction, we have stated explicitly that we utilize both heuristics and optimal control.
  ...
  We have updated the problem formulation, clearly stating that the objective is to find an energy-aware trajectory of parameters rather than optimal.
...
\begin{formal}
  ...
  \begin{formal}
    \color{black} [...] The set of parameters is unaltered through{\color{blue}out} the flight, i.e, $c_i:=\begin{bmatrix}c_{i,1}&c_{i,2}\end{bmatrix}',\forall i${\color{blue}, along $\delta_i$ %utilized 
...
\begin{formal}
  ...
  the $k$th computation parameter constraint set. {\color{blue}Indices $j,k$ serve~to~differentiate path and computation parameters constraints and indicate that each parameter can have a different constraint set.}
  \vspace*{1ex}
\end{formal}
...
$A$ is then a skew-symmetric matrix that contains sub-matrices $A_j$ utilized for the periodic behavior with the frequencies $\omega:=2\pi/T$ ($T$ is the period). Each frequency is multiplied by $j$ from one up to the order $r$, where $j$ is the index of the sub-matrix, i.e., there are $r$ frequencies modeling the periodicity and the base frequency.
...
\begin{formal}
  ...
  models in Eqs.~({\color{red}2}--{\color{red}3}) {\color{black}are equal when $\mathbf{u}$ is a {\color{black}zero vector %, matrices $A,C$ described by Eqs.~(\ref{eq:mat_A}--\ref{eq:mat_C}), 
  and an initial guess $\mathbf{q}(t_0)=\mathbf{q}_0$ at {\color{blue}the} initial time instant $t_0$}}
...
Thank you for underlying the necessity of further explanation for the steps in $\mathcal{K},\mathcal{T}$ in both points 7 and 8. The step for $\mathcal{T}$ is utilized to decide the granularity of re-planning, i.e., how often the aerial robot finds the configurations of path and computations parameters, whereas the step in $\mathcal{K}$ denotes the integration step for the models in MPC.
...
Thank you once again for underlying this point. The assumption was not clearly stated in the manuscript, and we have thus added additional commentary before the equations.
...
\end{formal}
  ...
  \begin{formal}{\color{black}
    [...] The energy optimization of computations schedules can be achieved by, e.g., varying the quality of service between specific bounds~[{\color{green}10}] and frequency and voltage of the computing hardware~[{\color{green}7}], 
    {\color{black}[{\color{green}11}], [{\color{green}12}]. We focus on the former aspect and schedule the onboard computations altering their quality while simultaneously changing the quality of the coverage.}
    {\color{blue} Concretely, we alter how often the aerial robot detects ground patterns along with the distance of the lines that form the coverage.} \color{black}[...]}
    \vspace*{1ex}
  \end{formal}
  ...
  Thank you for noticing the comparison was missing in the manuscript. We utilize as a performance metric the weighted average configuration of parameters divided by the battery state of charge at the end of the flight both expressed in percent, meaning the higher the average value of parameters and the lower the battery state of charge the better (all the resources are being utilized). For example, if the parameter is the detection and it varies between 2 and 10 frames per second, 2 corresponds to 0, whereas 10 to 100. Similarly, 0 then corresponds to no and 100 to full charge for the battery state of charge. We have added explanatory text to the simulation results to report the metric for i and ii against I and II respectively.
...
\end{formal}
  ...
  We have then reorganized the conclusion, underlining the intent to perform physical experiments in future instances of our work.
...
\begin{formal}
  \color{black}
  [...] It is similar to another motion in the literature, the \emph{Zamboni motion}~[{\color{green}40}], but additionally allows variable CPP %at the very core of this work
...
\begin{formal}
  \color{black} [...] Such use cases arise in precision agriculture~[{\color{green}4}] where {\color{blue}information collection prior to a harvesting operation and} damage prevention during the operation involve aerial robots~[{\color{green}5}],~[{\color{green}6}].
...
%We would like to propose a revised version of the manuscript for your kind consideration, where we incorporate the points and additional experimental results.}
}
