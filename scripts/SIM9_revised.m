...
% simulation of the algorithm, path II, dynamic, revised MPC
            ...
            if and(mod(k,1/delta_T) == 0,...  % MPC runs every second
                ...
                for u_mpc = U.' % iterate all the controls
                    ...
                    for j = k+1:k+N
                        ...
                        uk = [];
                        if length(u_mpc) >= u_mpc_count
                            uk = [c1;u_mpc(u_mpc_count)];
                            eeeu = est_u(uk(1),uk(2));
                        end
                                                   % 1 sec evolution
                        for jj = delta_T:delta_T:1 % with euler-transc.
                            if j <= k+N-1
                                qq1 = Ad*qq0+B*u(eeeu,eeu);
                                              % evolve with control for the
                                              % cost l
                                eeu = eeeu;
                            else
                                qq1 = Ad*qq0; % evolve with no control 
                                              % for the cost Vf
                            end
                            yy1 = C*qq1;
                            b0 = b0+delta_T*b(yy1);
                        ...
                        if yy1 > b0*qc*int_v % if control is not admissible
                                             % penalizing L
                            now_L = -1;
                            break;
                        else
                        ...
                        u_mpc_count = u_mpc_count+1;
                    end
                    
                    L = [L;now_L];
                    
                    if now_L == -1
                        continue; % next control; current not admissible
                    end
                end
                
                if all(L == -1) % all values -1
                    c2 = min_c2;
                else
                    [max_L max_idx] = max(L);
                    c2_chain = U(max_idx,:); % corresponds to argmax
                    c2 = c2_chain(1); % getting the next step control
                end
