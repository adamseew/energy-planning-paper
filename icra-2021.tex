...
% Robots critically depend on battery for the completion of their mission, the duration of which is proportional to the state of the power source

% Modeling mechanical and computational energy separately allows tradeoffs between the two to be expressed, such as performing additional onboard computations to reduce the amount of mechanical energy needed to complete the mission.

% The increasing demand for autonomy, challenge the traditional computational model and perhaps justify the presented approach.

% Preliminary data using a terrestrial fixed-wing craft indicates a potential mission extension of up to 13 minutes over 1 hour flight by switching to the lowest QoS, with the computations accounting for up to 23% of the total energy. The approach ideally accounts for the extension by adapting the trajectory and the computations as the robot batteries drain. 
...
Given a position $\mathbf{p}\in\mathbb{R}^3$ in the 3D Euler space, with respect to some inertial navigation frame $\mathcal{O}_W$, a guidance action--which affects the position allowing the motion along the 3-axis--is built upon the direction to follow, derived using vector field~\cite{de2017guidance}.
