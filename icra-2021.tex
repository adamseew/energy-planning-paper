...
\begin{figure*}
  \begin{tikzpicture}
    ...
    \path[fill=black] (0.0000,0.0000) node[above right] () at (-3.1,-4) {\footnotesize {(a) slice of the simulated energy along the estimated model}};
    \node[inner sep=0pt] () at (10.6,.05)
        {\footnotesize\input{\figpath/results/coef_sim3D.tikz}};
    \path[fill=black] (0.0000,0.0000) node[above right] () at (4.3,-4) {\footnotesize {(b) model evolution at different $k$s}};
    \node[inner sep=0pt] () at (0,0)
        {\footnotesize\input{\figpath/results/ener_sim3_detail.tikz}};
    \path[fill=black] (0.0000,0.0000) node[above right] () at (9.6,-4) {\footnotesize {(c) coefficients evolution}};
  \end{tikzpicture}
  \caption[] {Evolution of the simulated energy data from Figure~\ref{fig:res1}, using the estimation first in~\ref{}, the data with no estimation predicting the future in~\ref{}, and evolution of the estimated coefficients in~\ref{}}
\end{figure*}
