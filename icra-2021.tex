...
\begin{proof}
  ...
  \emph{(Induction step)}. By inspection of Equation~(\ref{eq:state-perf}), the output at instant $k$ can be expressed $y_{k}=(\alpha_{0,0}+B\mathbf{u}_0+\dots+B\mathbf{u}_{k-1})+p_1(k)\alpha_{0,1}+\dots+p_r(k)\alpha_{0,r}$, where by omitting the sign
  \begin{equation}
    p_r(k):=\begin{cases}
