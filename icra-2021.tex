...
---
%up is ok; down fixes to the text; main idea is to simplify how the plan is defined!

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Mission Planning Overview}
\label{sec:prob}

We assume that the initial plan consists of different stages. At each stage the robot must follow the TEE within TEE sets and do the computations within the computations sets.
...
\begin{defn}[Stage and plan]\label{def:mission}
  A \emph{stage} $\mathcal{M}_i$ at time instant $k$ of a plan $\mathcal{M}$ is defined as the ordered list
  ...
  The \emph{plan} is then $\mathcal{M}:\mathbb{R}^2\rightarrow\mathcal{M}_i$ a function which maps the point $\mathbf{p}_k$ to a specific stage $\mathcal{M}_i$.
  
  For simplicity the system is sampled in discrete time. The algorithm takes as input $\mathcal{M}$, initial position, and energy coefficients guess, and outputs the new position, the instantaneous energy consumption, and the control input--an action performed evolving the plan state.
\end{defn}
