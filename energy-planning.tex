...
\author{
  ...
  \thanks{The work was partly funded by EU grant \textnumero 779882 (TeamPlay). The work for H.\vspace*{.4ex}G. is supported by the Ramon y Cajal grant \textnumero RYC2020-030090-I.}
  \thanks{$^{\text{1}}$A.\hspace*{.4ex}S. is with the Department of Mechanical Engineering and Material Science, Yale University, CT, USA, but the work was performed while affiliated with the SDU UAS. Email: {\tt\footnotesize \href{mailto:adam.seewald@yale.edu}{adam.seewald@yale.edu}};}
...
\begin{figure}[t]
 ...
 %\end{tikzpicture}
  ...
  {\color{black}Top-right} %shows 
  {\color{black}are} the energy data of a %physical 
  fixed-wing aerial robot flying a static coverage plan similar to the one illustrated here{\color{black}; %below is 
  }%. Bottom-right shows 
...
measured in watts, is derived from Eq{\color{black}s}.~(\ref{eq:batdyn}--\ref{eq:batdyn2}), i.e., the computation parameters in Algorithm~\ref{alg2} and Eq.~(\ref{eq:ocp-output-mpc}) later in Sec.~\ref{sec:repla-algo} will have an energy constraint.
%\end{defn}
...
\begin{figure}[t]
  ...
  \begin{minipage}[l]{0.26\columnwidth}
    \caption{Zamboni-li- ke motion: $\overline{\Gamma}$ with four primitive paths (Lines~\ref{alg2:circ1}--\ref{alg2:trig4} in Algorithm~\ref{alg2}) can be re-planned ($\underline{\Gamma}$) {\color{black}via} $r_2$.% in Eq.~(\ref{eq:r2}).
    }
    \label{fig:zambo}
  \end{minipage}
  \vspace*{-5ex}
...
optimization {\color{black} as well as heuristics-based} approaches~\cite{brateman2006energy,zhang2007low,ondruska2015scheduled,lahijanian2018resource}. We similarly derive an optimal control problem {\color{black}and a greedy approach} returning the trajectory of parameters $c_i(\mathcal{T})$ with $\mathcal{T}:=[t_0,t_f]$ (see Definition~\ref{def:meas}). Since the final time %instant 
and the %exact 
...
{\color{black} $c_i^\rho$} %and respects the constraints
...
implemented using ROS. The planner varies the path parameter $c_{i,1}$ %in %Eq.~(\ref{eq:r2}) 
