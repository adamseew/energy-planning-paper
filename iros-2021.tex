...
\begin{figure}[ht]
  ...
  \caption[] {The models after 200 seconds on top. The algorithm that uses the model I to dynamically adapt the path (when the final time and battery time doesn't match) and the computation (when the battery is discharged--the smaller plot on right) on bottom}
  \label{fig:ener}
\end{figure}
