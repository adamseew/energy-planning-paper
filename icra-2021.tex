...
\begin{figure}[h!]
  ...
  \caption[Path of a static and dynamic plan]{Path simulations with variations of wind speed and direction. In I, II the path is static. It is dynamically adapted with the algorithm in I.d, with the path parameter radius of the circle $c_1$ and computation parameter frames-per-second rate $c_2$}
  \label{fig:trajs}
\end{figure}
\begin{figure}[h!]
  \footnotesize
  \input{\figpath/results/new_physics/model.tikz}
  \caption[Energy estimation and evolution of the state]{Energy estimation process for the first 6 seconds on the left side, the evolution of the state $\mathbf{q}$ on the right}
  \label{fig:model}
\end{figure}
\begin{figure}[h!]
  \footnotesize
  \input{\figpath/results/new_physics/ener.tikz}
  \caption[] {The model after 200 seconds of estimation on top along the energy and the algorithm that dynamically adapts path when the UAV cannot reach the end of the plan, and adapts the computation when the battery is discharged (smaller plot on right)}
  \label{fig:ener}
\end{figure}
