...
\begin{algorithm}[t]
  ...
  \begin{algorithmic}[1]
    \STATE $\mathbf{p}_{k}\gets \mathbf{p}_0, \mathbf{q}_{k}\gets \mathbf{q}_0$
    \STATE $\mathbf{u}_{k}\gets \overline{c}_i$\label{alg:init}
    \FOR{$i\in[l]^+$}\label{alg:stages-loop}
      \WHILE{$\mathbf{p}_{k}\neq\mathbf{p}_{\Gamma_i}$}\label{alg:while}
        \STATE $\mathbf{u}_{k+1}\gets\argmax_{\mathbf{u}}\hspace*{-1ex}{\displaystyle{\sum\limits_{j=k+1}^{k+N-1}}\hspace*{-1ex}{l(\mathbf{q}_j,\mathbf{u}_j)+V_f(\mathbf{q}_{k+N})}}$\label{alg:mpc}
        \STATE $\hat{\mathbf{q}}_{k+1}\gets A\mathbf{q}_{k}+B\mathbf{u}$\label{alg:evolution}
        \STATE $\mathbf{q}_{k+1}\gets$ the estimate of the state from $\hat{\mathbf{q}}_{k_1}$ and sensor data\label{alg:kalman_end}
        \STATE $\mathbf{p}_{k+1}\gets\mathbf{p}_{k}\dot{\mathbf{p}}_d(\mathbf{p}_{k})/v$ \label{alg:pos}
        \STATE $k\gets k+1$
      \ENDWHILE
    \ENDFOR
  \end{algorithmic} 
