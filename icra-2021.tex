...
We further note that at every step of the summary on line~\ref{alg:mpc} the algorithm has to evolve the state to check if the output satisfies the output constraint set, and if the control satisfies the trajectory and the computation constraint sets. In particular, at each steps it performs the subroutine
\begin{algorithmic}
...
We finally note that one can express the tradeoffs between trajectory and computation parameters (e.g., a decrement in the distance between the lines in a survey scenario is related to a decrement in the number of detections per second) enriching the control constraint set with
\begin{equation*}
  F_i\mathbf{u}_k-e_i\geq 0,
\end{equation*}
where $e_i\in\mathbb{R}^n$ and $F_i\in\mathbb{R}^{n\times n}$ expresses the relation between the parameters (if $F_i$ is an identity matrix there is no relation between the parameters).
