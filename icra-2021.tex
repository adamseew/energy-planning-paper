...
Often, unmanned aerial vehicle (UAV) planning algorithms generate a trajectory. We are interested in a different algorithm: one that plans a UAV mission. In such a mission, the UAV has to fly an assigned path--which can be composed of many trajectories--and compute some on-board computational tasks. Many scenarios involving UAVs, such as precision agriculture, search and rescue, and surveillance, necessitates mission planning, and are energy critical. For instance, the UAV might detect ground patterns and inform other ground-based actors with little human interaction. We refer to the computational tasks that can be dynamically replanned as computations. We are interested in energy-aware dynamic mission planning, which would find optimal tradeoffs between the path, computations, and energy requirements. The UAV can deviate from the path and change the computations within user-defined constraints. Current generic planning solutions for outdoor UAVs do not plan the mission dynamically, nor are energy-aware. They are often semi-autonomous; the mission is static and usually defined using mission planning software~\cite{daponte2019review}. It is such a state of practice that has prompted us to propose an \emph{energy-aware dynamic mission planning algorithm} for UAVs. The algorithm combines and generalizes some of the past body of knowledge on mobile robot planning problems and addresses the increasing \emph{computational demands} and their relation to energy consumption, path, and autonomy for the UAV mission planning problem. 
