...
\begin{assm}[Plan periodicity]\label{assm:periodic}
  Given an initial plan $\Gamma$ consisting of $l$ stages $\Gamma_i$ s.t. $i\in[l]^+$ and a generic starting point $\mathbf{p}_k$, there exists an integer positive constant $k<l$ such that
  \begin{equation*}
    \varphi_i-\varphi_{k+i}=c_k,
  \end{equation*}
  where $c_1,\dots,c_k$ are arbitrary constant differences.
\end{assm}

Physically, this means that the plan is periodic, with periodicity $k$. From Assumption~\ref{assm:periodic}, follows the next lemma.

\begin{lem}[Rank of the trajectory functions]
  Suppose Assumption~\ref{assm:periodic} holds. Then
  \begin{equation*}
    \mathrm{rank}\begin{bmatrix}
        \varphi_1 & \varphi_{k+1} & \cdots & \varphi_{(l-k)+1} \\ 
        \varphi_2 & \varphi_{k+2} & \cdots & \varphi_{(l-k)+2} \\
        \vdots & \vdots & \ddots & \vdots \\
        \varphi_k & \varphi_{2k} & \cdots & \varphi_l   
      \end{bmatrix}=2.
  \end{equation*}
\end{lem}
\begin{proof} 
  The proof follows directly from Assumption~\ref{assm:periodic}. The column rank of the matrix in the lemma is the dimension of its column space. This denotates the size of the set of all the possible linear combination of the column vectors. We note that there is a limited number (two) of linear combinations of the column vectors being these linearly dependent (the values of the constant differences $c_1,\dots,c_k$ are the same for each $i$). 
\end{proof}

The lemma shows an easy way to verify that a plan is periodic.

\begin{pb}[UAV planning problem]
  Consider an initial plan $\Gamma$ satisfying Assumption~\ref{assm:periodic}. We are interested in the planning of the path parameters $c_{i,1},\dots,c_{i,\rho}$ and computations parameters $c_{i,\rho+1},\dots,c_{i,\rho+\sigma}$ for the UAV planning problem.
\end{pb}
