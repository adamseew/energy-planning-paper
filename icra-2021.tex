...
\begin{figure}[h]
  ...
  \caption{The notation on a small initial slice of the plan in Figure~\ref{fig:il-abs} in 2D.}
  \label{fig:traj1}
\end{figure}
...
\begin{equation*}\label{eq:pd}
  ...
  \end{bmatrix},
\end{equation*}
where $E$ specifies the rotation (it influence the tracking direction), and $k_e\in\mathbb{R}_{\geq 0}$ the gain to adjusts the speed of convergence. The direction the velocity vector $\dot{\mathbf{p}}_d$ is pointing at is generally different from the course heading $\dot{\mathbf{p}}$ due to the atmospheric interferences, such as wind ($w\in\mathbb{R}$ in the top of Figure~\ref{fig:tee1}).
...
\begin{defn}[Output constrain set]
...
\begin{algorithm}
  \begin{algorithmic}[1]
    \STATE $\mathbf{p}_k\gets \mathbf{p}_0, \mathbf{q}_k\gets \mathbf{q}_0$
    \FOR{$i\in[l]^+$}
      \WHILE{$\mathbf{p}_k\neq\mathbf{p}_{\Gamma_i}$}
        \STATE $\mathbf{u}_k\gets\argmax_{\mathbf{u}}\hspace*{-1ex}{\displaystyle{\sum\limits_{i=k-1}^{k+N-2}}\hspace*{-1ex}{l(\mathbf{q}_i,\mathbf{u}_i)+V_f(\mathbf{q}_{k+N-1})}}$\label{alg:mpc}
        \STATE $\mathbf{u}\gets\{\mathbf{u}\cup\mathbf{u}_k\}$
        \STATE $\hat{\mathbf{q}}_{k+1}\gets A\mathbf{q}_k+B\mathbf{u}_k$\label{alg:evolution}
        \STATE $\mathbf{q}_{k+1}\gets$ the estimate of the state from $\hat{\mathbf{q}}_{k+1}$ and sensor data\label{alg:kalman_end}
        \STATE $\mathbf{p}_{k}\gets\mathbf{p}_k\dot{\mathbf{p}}_d(\mathbf{p}_k)/v$ \label{alg:pos}
        \STATE $k\gets k+1$
      \ENDWHILE
    \ENDFOR
  \end{algorithmic}
  \caption{Energy-Aware Dynamic Planning}    
\end{algorithm}
