...
\begin{equation}\label{eq:state-control}\begin{split}  
  ...
  \end{array}\right],
\end{split}\end{equation}
where $\mathbf{s}_k$ is defined in Subsection~\ref{sec:computations-model}, $\mathbf{u}_k\in\mathbb{R}^l$ is the control given $l:=1+\rho$, and $B\in\mathbb{R}^{j\times l}$. Moreover, the first item is one, while the others on the first row are gain factors $\omega_{k}\in\mathbb{R}$, quantifying the contribution of a given adjustment to the instantaneous energy consumption. 
...
In the case of the system behaving ideally (i.e., with no uncertainty), we expect a state (energy coefficients) evolving accordingly to its output (instantaneous energy consumption). An observation summarized in the following Lemma.
