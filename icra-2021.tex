...
We assume that a user-specified initial plan consists of different stages. At each stage, the UAV must follow the TEE within TEE sets and do the computations within the computations sets.

Let us suppose the TEE at stage $i$ can be altered with $\rho$ TEE parameters and the computations with $\sigma$ computations parameters. We then express the TEE as a continuous twice differentiable function $\varphi_i:\mathbb{R}^2\times\mathbb{R}^\rho\rightarrow\mathbb{R}$ of a point and the TEE parameters which returns a metric of the distance between the point and the nominal trajectory. We express and the computations as the value of the computations parameters. We discuss the concrete meaning of the value of TEE parameters in Subsection~\ref{sec:model}), and the of the computations parameters in Subsection~\ref{sec:computations-model}. 

Let us further adopt the following mathematical notation.  $[a]$ is the set $\{0,1,\dots,a\}$, $[a]^+$ the set $[a]/\{0\}$, $\langle a_1,a_2,...,a_n\rangle$ an ordered list of $n$ elements, $c^0$ the optimal value of parameter $c$,and $\underline{c},\overline{c}$ its lower and upper bound retrieved from a plan specification. The plan specification is a file which specifies all the stages and triggering points along the plan. We discuss the format in Subsection~\ref{sec:plan}. Moreover, we use the notation $c_{i,j}$ to specify the $j$-th item of the $i$-th parameters set $c_i$.
...
\begin{defn}[Stage, plan and triggering point]\label{def:mission}
  ...
  \end{tikzpicture}
  \caption{Finite state machine of the plan}
  \label{fig:state-machine}
  \end{figure}

  A slice of a finite state machine in Figure~\ref{fig:state-machine} shows the transition between the stages. The triggering point $\mathbf{p}_{i-1}$ allows the transition to stage $\Gamma_i$. The UAV remains in the stage with any generic point $\mathbf{p}_k$. It eventually enters the stage $\Gamma_{i+1}$ with the triggerint point $\mathbf{p}_i$ and so on, until it reaches the last triggering point.

  The algorithm takes as input the plan, the triggering points (using the plan specification) and some initial guesses that are necessary for the position and energy estimation. It outputs the new position, the instantaneous energy consumption, and the control input--an action performed evolving the state.
\end{defn}

\subsection{Plan specification file format}
\label{sec:plan}

We specify the stages $\bigcup_i{\Gamma_i}$, triggering points $\bigcup_i\mathbf{p}_{\Gamma_i}$, and the plan $\Gamma$ using a specific file format--the plan specification. We refer the reader to Appendix~\ref{app:plan-example} for an example.

The first stage $\Gamma_1$ is specified as the first line of the file
\begin{algorithmic}[1]
  \State\textbf{\texttt{$k_e$;$r$;$\varphi_1(\mathbf{p}_k,c_{1,1},\dots,c_{1,\rho})$}}\label{code:stage}
\end{algorithmic}
where $k_e$ is a convergence value, $r$ a rotation, and $\varphi_1$ the TEE. We refer the reader to the next subsection where we discuss the physical meaning of the convergence value and rotation. The rotation is expressed in binary format with zero being the counter-clockwise rotation, one the clockwise rotation.
...
\end{algorithmic}
where $x$ and $y$ are the two coordinates of the point. The UAV reaches the point within a given radius (a parameter of the algorithm) and enters the next stage $\Gamma_2$. The coordinates can be expressed in function of the TEE parameters $c_1$. Generally, one can express the coordinates in function of any TEE parameters $c_i$, propagating the information therein if necessary (see Figure~\ref{fig:state-machine2}). 

\begin{figure}[h]
  \center
  \begin{tikzpicture}[shorten >=1pt,node distance=24ex,on grid,auto] 
    \node        (q_dots0) {$\cdots$};
    \node[state] (q_0) [right=of q_dots0] {$\langle\varphi_i,c_i\rangle$};
...
We refer the reader to the example in Appendix~\ref{app:plan-example} for a detailed implementation. In the example, the first TEE parameter is propagated to all the following TEEs and triggering points. 

The following $\rho+\sigma$ lines
\begin{algorithmic}[1]
  \setcounterref{ALG@line}{code:trigger-pt}
  \State\textbf{\texttt{[$\underline{c}_i$,$\overline{c}_i$]}}\label{code:parameter-set}
\end{algorithmic}
specifies the lower $\underline{c}_i$ and upper $\overline{c}_i$ bound of the $i$-th parameter set. They are in ascending order and the TEE parameters sets are specified before the computations parameters sets.
...
Let us assume the set
\begin{equation}\label{eq:area}
  \mathcal{P}_i:=\{\mathbf{p}_k\mid\varphi_i(\mathbf{p}_k,c_{i,1},\dots,c_{i,\rho})\in\mathbb{C}_i\},
\end{equation}
delimits the area where the $i$-th TEE $\varphi_i$ is free to evolve using $\rho$ adjustments $\mathbf{c}_i:=c_{i,1},...,c_{i,\rho}$ (the gray area in Figure~\ref{fig:overview}). Phisically, $\varphi_i$ is s function of the two coordinates and is satisfied when a point $\mathbf{p}_k$ is on the TEE. Physically, this means the UAV is flying exactly over the nominal trajectory. The TEE parameters allows to change the TEE. They are a way to alter the nominal trajectory in the initial plan.

In fact, the algorithm uses the set from Equation~(\ref{eq:area}) to alter the optimal TEE parameters $\mathbf{c}_i^0$, so $\varphi_i(\mathbf{p}_k,\mathbf{c}_i^0)$ has the highest instantaneous energy consumption (while still respecting the energy budget). It guides the UAV to the new position $\mathbf{p}_{k+1}$ using the function $\varPhi:=\varphi_i(\mathbf{p}_k,\mathbf{c}_i^0)$, computing its vector field $\nabla\varPhi:=(\partial\varPhi/\partial x,\partial\varPhi/\partial y)$, and deriving the direction to follow in the form of velocity vector
...
% fixed up to here!
% idea; write control as parameters; write B as time-dependent, which multiplied TEE parameters gives energy due to trajectory (approx). By computations due to computations (from powprof).

\subsection{State: energy $\mathbf{q}$}
\label{sec:energy-model}

We model the energy using some energy coefficients $\mathbf{q}\in\mathbb{R}^m$ that characterize the energy signal. We refer to the instantaneous energy consumption evolution simply as energy signal. The coefficients are derived from Fourier analysis (the size of the energy coefficients vector $m$ is related to the order of a Fourier series) and decomposes the evolution in energy due to the path and computations. This approach is adapted from our earlier work on computational energy analysis~\cite{seewald2019coarse, seewald2019component}, and energy estimation of a fixed-wing UAV~\cite{seewald2020mechanical}. 

We show a relation between the energy signal and the energy coefficients in Theorem~\ref{thm:state-vs-energy}, but the two are different. We show after the main results how this approach allows us variability in terms of the systems behaving periodically, piece-wise periodically, or merely linearly with sporadic periodicity.
...
\end{equation}
where $h:\mathbb{R}_{\geq 0}\rightarrow\mathbb{R}$ maps time to the signal, $\omega:=2\pi/T$ is the angular frequency, and $a,b\in\mathbb{R}$ the Fourier series coefficients.
...
\begin{equation}\label{eq:state-details}\begin{split}
  ...
  \end{array}\right],
\end{split}\end{equation}
where $\mathbf{q}\in\mathbb{R}^m$ with $m=2r+1$, $A\in\mathbb{R}^{m\times m}$ is the state transmission matrix, and $C\in\mathbb{R}^m$ is the output matrix. In matrix $A$, the top left entry is one, the diagonal entries are $A_1,\dots,A_r$, the remaining entries are zeros.

\begin{lem}[Signals equality]\label{lem:eqv}Suppose there is no state and output uncertainty (the contributions $\mathbf{w}\in\mathbb{R}^m,v\in\mathbb{R}$ are zeros) and the control is zero. Suppose further matrices $A,C$ are described by Equation~(\ref{eq:state-details}), and the initial guess $\mathbf{q}_0$ is $\mathbf{q}_0=\begin{bmatrix}a_0 & a_1/2 & b_1/2 & \cdots & a_r/2 & b_r/2\end{bmatrix}$. Then Equation~(\ref{eq:fourier}) describes the same signal as Equation~(\ref{eq:state-perf}).
\end{lem}
\begin{proof}
The equivalence of the models is trivial and the equality of the two signals is achieved by a proper choice of items of matrices $A,C$ and the initial guess $\mathbf{q}_0$. We refer the reader to Appendix~\ref{app:proof-eqv} for a formal proof of the equality.
\end{proof}

Let us consider the discretized version $A_d:=A\Delta t+I$ of the matrix $A$. We denote the continuous and discrete time with the standard notation $t,k$. We use the forward Euler approximation with a small enough value of the time step $\Delta t$. One can guarantee to have the same outputs rather than an approximation with $A=e^{At}$.

Let us suppose that at time instant $k$ the plan reached $i$-th stage. The control along with the input matrix
...
Equation~(\ref{eq:state-control}) accounts for the energy due to the change of parameters $\mathbf{u}_k-\mathbf{u}_{k-1}$. For instance, when the TEE $\varphi_0$ is a circle (see Figure~\ref{fig:overview}), a decrement in the TEE parameter $c_{0,1}$--the radius of the circle--adds a negative contribution. It thus simulates the lowering of instantaneous energy consumption $\nu_{0,1}(c_{0,1}-c_{0,1}^-)<0$. The first $\rho$ gain factors quantifies the contribution of the $i$-th TEE parameters. A first guess of these parameters is derived empirically. They are later refined by the algorithm. The following $\sigma$ gain factors quantifies the contribution of the $i$-th computations parameters. They are derived using the modeling tool. We use the gain factors to select the optimal control $\mathbf{u}^0$. 
